...
test('site-wide navigation hides Hot Takes and exposes Games before About', async () => {
  ...
  assert.match(config, /identifier\s*=\s*"games"[\s\S]*?name\s*=\s*"Games"[\s\S]*?url\s*=\s*"\/games\/"[\s\S]*?weight\s*=\s*5/);
