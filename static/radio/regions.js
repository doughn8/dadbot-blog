// Dadbot Radio — continent lookup for the Region -> Country browser tree.
// Plain script (loaded before radio.mjs); exposes window.DADBOT_REGIONS.
// Maps ISO 3166-1 alpha-2 codes to the six populated continents; anything
// unmapped falls into "Other" at render time. (Roadmap item 19.)
(function () {
  'use strict';

  var NORTH_AMERICA = [
    'AG', 'AI', 'AW', 'BB', 'BL', 'BM', 'BQ', 'BS', 'BZ', 'CA', 'CR', 'CU',
    'CW', 'DM', 'DO', 'GD', 'GL', 'GP', 'GT', 'HN', 'HT', 'JM', 'KN', 'KY',
    'LC', 'MF', 'MQ', 'MS', 'MX', 'NI', 'PA', 'PM', 'PR', 'SV', 'SX', 'TC',
    'TT', 'US', 'VC', 'VG', 'VI',
  ];
  var SOUTH_AMERICA = ['AR', 'BO', 'BR', 'CL', 'CO', 'EC', 'FK', 'GF', 'GY', 'PE', 'PY', 'SR', 'UY', 'VE'];
  var EUROPE = [
    'AD', 'AL', 'AT', 'AX', 'BA', 'BE', 'BG', 'BY', 'CH', 'CY', 'CZ', 'DE',
    'DK', 'EE', 'ES', 'FI', 'FO', 'FR', 'GB', 'GG', 'GI', 'GR', 'HR', 'HU',
    'IE', 'IM', 'IS', 'IT', 'JE', 'LI', 'LT', 'LU', 'LV', 'MC', 'MD', 'ME',
    'MK', 'MT', 'NL', 'NO', 'PL', 'PT', 'RO', 'RS', 'RU', 'SE', 'SI', 'SK',
    'SM', 'UA', 'VA', 'XK',
  ];
  var AFRICA = [
    'AO', 'BF', 'BI', 'BJ', 'BW', 'CD', 'CF', 'CG', 'CI', 'CM', 'CV', 'DJ',
    'DZ', 'EG', 'EH', 'ER', 'ET', 'GA', 'GH', 'GM', 'GN', 'GQ', 'GW', 'KE',
    'KM', 'LR', 'LS', 'LY', 'MA', 'MG', 'ML', 'MR', 'MU', 'MW', 'MZ', 'NA',
    'NE', 'NG', 'RE', 'RW', 'SC', 'SD', 'SH', 'SL', 'SN', 'SO', 'SS', 'ST',
    'SZ', 'TD', 'TG', 'TN', 'TZ', 'UG', 'YT', 'ZA', 'ZM', 'ZW',
  ];
  var ASIA = [
    'AE', 'AF', 'AM', 'AZ', 'BD', 'BH', 'BN', 'BT', 'CN', 'GE', 'HK', 'ID',
    'IL', 'IN', 'IQ', 'IR', 'JO', 'JP', 'KG', 'KH', 'KP', 'KR', 'KW', 'KZ',
    'LA', 'LB', 'LK', 'MM', 'MN', 'MO', 'MV', 'MY', 'NP', 'OM', 'PH', 'PK',
    'PS', 'QA', 'SA', 'SG', 'SY', 'TH', 'TJ', 'TL', 'TM', 'TR', 'TW', 'UZ',
    'VN', 'YE',
  ];
  var OCEANIA = [
    'AS', 'AU', 'CK', 'FJ', 'FM', 'GU', 'KI', 'MH', 'MP', 'NC', 'NF', 'NR',
    'NU', 'NZ', 'PF', 'PG', 'PN', 'PW', 'SB', 'TK', 'TO', 'TV', 'VU', 'WF', 'WS',
  ];

  var CONTINENTS = {
    'North America': NORTH_AMERICA,
    'South America': SOUTH_AMERICA,
    Europe: EUROPE,
    Africa: AFRICA,
    Asia: ASIA,
    Oceania: OCEANIA,
  };

  var ROOT =
    typeof window !== 'undefined'
      ? window
      : globalThis;

  ROOT.DADBOT_REGIONS = {
    REGION_NAMES: Object.keys(CONTINENTS),
    CONTINENTS: CONTINENTS,
    regionFor: function (countryCode) {
      var code = String(countryCode || '').toUpperCase();
      for (var name in CONTINENTS) {
        if (CONTINENTS[name].indexOf(code) !== -1) return name;
      }
      return 'Other';
    },
  };
})();
